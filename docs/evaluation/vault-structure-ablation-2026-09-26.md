...
For multi-hop recall, keep navigation pages and their links intact. Treat a MOC as a routing node, then inspect linked source notes for answer evidence. The adaptive graph path omits `canonical_memory: false` pages from returned answer candidates while retaining them as traversal nodes; its assessor cannot cite one alone as sufficient evidence. Ordinary `recall` and `recall-loop` now keep navigation pages in the ranked corpus but omit them from answer slots by default. `--include-navigation` shows them when the index itself is the target. The new follow-up eval was authored from the same small vault, so this is not a general retrieval claim.
