...
For multi-hop recall, keep navigation pages and their links intact. Treat a MOC as a routing node, then inspect linked source notes for answer evidence. The experimental adaptive graph path now omits `canonical_memory: false` pages from returned answer candidates while retaining them as traversal nodes; its assessor cannot cite one alone as sufficient evidence. A two-hop test covers this behavior. The ordinary recall commands have not changed their candidate policy. This is not yet a held-out retrieval win.
